...
external_document_dir = os.path.join(current_dir, 'external_vi')
...
def get_data_from_txt_files(folder_path, max_length=100):
  documents = []
...
def generate_embeddings(texts, max_length=100):
  all_embeddings = []
  for text in texts:
...
# BƯỚC 8:
# Khởi tạo chatbot
def initialize_chatbot(external_document_dir, max_length=100):
  initialize_models()

  global documents
  documents = get_data_from_txt_files(external_document_dir, max_length)

  global doc_embeddings
  segmented_documents = [segment_words(document) for document in documents]
  doc_embeddings = generate_embeddings(segmented_documents)

initialize_chatbot(external_document_dir)
...
def update_output(n_clicks, user_message, chat_history):
  if n_clicks > 0 and user_message:
    ...
    for document in top_k_documents:
      ...
      # Print the document
      print(document_text + '\n')
    ranked_docs = re_rank_documents(segmented_user_message, top_k_documents)
